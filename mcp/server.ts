#!/usr/bin/env tsx
/**
 * Discord MCP server for Cursor (ported from claude-plugins-official).
 *
 * Self-contained MCP server with full access control: pairing, allowlists,
 * guild-channel support with mention-triggering. State lives in
 * ~/.cursor/channels/discord/access.json — managed by the /discord:access skill.
 *
...
const MCP_ONLY = process.env.CDC_MCP_ONLY === '1'
  ...
  for (const senderId of files) {
    ...
    void (async () => {
      try {
        ...
        if ('send' in ch) {
          await ch.send('Paired! Say hi to your Cursor agent.')
        }
        rmSync(file, { force: true })
      } catch (err) {
